...
# Process dev.server.conf with environment variables for dev
envsubst '$HOST_DOMAIN' < /nginx-config-source/dev-server.conf > /tmp/nginx-config/dev-server.conf
cp /tmp/nginx-config/dev-server.conf /etc/nginx/conf.d/dev-server.conf
